...
out.dir <- "/Users/hangxu/Projects_scg/ST/output3"
cpths <- list.files("/Users/hangxu/Projects_scg/ST/projects/BC/data/count/under_tissue", pattern="tsv", full.names=TRUE)
mpths <- list.files("/Users/hangxu/Projects_scg/ST/projects/BC/data/meta/feature_files", pattern="tsv", full.names=TRUE)
...
#ref.genes <- subset(genes.stat, exp.mean>0 & exp.var<5 & perc.non.zero>0.1)
...
ggsave("genes.stat.png")
write.table(genes.stat, file="genes.stat.tsv", row.names=FALSE, sep="\t", quote=FALSE)

mapped <- make_g2chr(genes=ref.genes$gene)
bins <-build_bins(bin.size=1e+7)
...
write.table(bin.ref, file="reference.tsv", sep="\t", quote=FALSE, row.names=FALSE)
...
## seg.data need columns:
## sample, start.gps, end.gps, var
...
p <- cnv.heatmap(segments, outfile="../cnv.heatmap.scaled.pdf", row.order=sample.subtype$sample)
ggsave(p,"../cnv.heatmap.scaled.pdf" )
# plot_grid(p, p.left, rel_widths=c(6,2))

# ggsave( "../cnv.heatmap.anno.pdf")
